...
def centile_compare(df1, df1_desc, variable, centile_length, df2=None, df2_desc=None):    
    ...
    if df2 is not None:
        ...
        df_centiles.plot(y=[df1_desc+' Sum'], kind='bar')
    plt.show()
    ...
    if df2 is not None:    
        ...
        df_2['bin_edge'] = bins2
        return (df_1, df1, df_2, df2)
    else:
        return (df_1, df1)

@numba.jit()
def calc_pit1(num_recs, totinc):
...
def calc_pit(income_array, rate, bracket):
    def taxamt(income, rate, bracket):
        ...
        tax_amount = (
            ...
            rate[8] * min(bracket[8] - bracket[7], max(0, income - bracket[7])) +
            rate[9] * min(bracket[9] - bracket[8], max(0, income - bracket[8])) +
            rate[10] * min(bracket[10] - bracket[9], max(0, income - bracket[9])) +            
            rate[11] * min(bracket[11] - bracket[10], max(0, income - bracket[10]))   
        )
        return tax_amount
...
def summary_stats(df, variable, rate, bracket, title):
    # Summary Statistics
    df[variable] = df[variable].astype(float)
    len_df = len(df)
    pitax = calc_pit(df[variable].values, rate, bracket)
    df = df.reset_index()
    df['tax_payer'] = np.where(df[variable]>bracket[1],1,0)
    df['pitax'] = pitax
...
filename = 'taxcalc/pit_mexico_sample.csv'
...
df_expanded = expand_df(df_income_all, 'tot_inc', 'id_n', 'weight')
df_expanded = df_expanded.sort_values(by='tot_inc')
...
# Plot single Dataset
plot_density_chart1(df_expanded, "HHS", 'tot_inc', "Density Plot for Mexico", "Income", logx=True, vline=None)

centile_length=20
ranking_col = "tot_inc"

centile_compare(df_expanded, 'Household Survey', 'tot_inc', centile_length)
...
r = 19
# fr is the number of observations in the last centile group(s)
# that needs to be replaced by a pareto distribution
fr=0 # initialize fr=0
for i in range(r, centile_length):
    fr = fr+ int(df_1.loc[i,'count_tot_inc'])

# Find the bin edge of the last replace bin
U = df_1.loc[r,'bin_edge']

# alternatively fr could be 
df_hhs_cut = df_expanded[df_expanded['tot_inc']>=U]
fr = len(df_hhs_cut)

s=13
alpha = calc_alpha(df_1, s, r,'tot_inc','median')
print('alpha: ',alpha)
#alpha = 1.0
# Generate the pareto distribution
x = generate_distribution(U, alpha, fr)
df_pareto = pd.DataFrame(x, columns =['tot_inc'])
df_pareto = df_pareto.sort_values(by='tot_inc')
df_pareto = df_pareto.rename(columns={'tot_inc': 'tot_inc_New'})

# comparing the cut portion and the pareto
df_hhs_cut = df_hhs_cut.rename(columns={'tot_inc': 'tot_inc_Old'})
df_hhs_cut1 = df_hhs_cut[['id_n', 'tot_inc_Old']].reset_index(drop=True)
...
df_hhs_cut_pareto['tot_inc']=np.where(df_hhs_cut_pareto['tot_inc_New']> 
                                                df_hhs_cut_pareto['tot_inc_Old'], 
                                                df_hhs_cut_pareto['tot_inc_New'], 
                                                df_hhs_cut_pareto['tot_inc_Old'])

# keep the rows from the lower percentiles in the Household Survey
df_hhs_kept = df_expanded[df_expanded['tot_inc']<U][['id_n','tot_inc']]
# attach the pareto adjusted values
df_hhs_new = pd.concat([df_hhs_kept, df_hhs_cut_pareto[['id_n', 'tot_inc']]], axis=0)
df_hhs_new = df_hhs_new.sort_values(by='tot_inc').reset_index()
df_hhs_new = df_hhs_new.drop('index', axis=1)

# New density plots for the adjusted distribution
plot_density_chart(df_expanded, "HHS", df_hhs_new, "HHS Adjusted", 'tot_inc', "Density Plot for Ethiopia", "Income", logx=True, vline=10.3)
...
df_1_updated, df_expanded, df_updated, df_hhs_new  = centile_compare(df_expanded, 'Household Survey', 'tot_inc', centile_length, df_hhs_new, 'Household Survey Updated') 
...
start = time.time()
rate = [0.0, 0.0192, 0.064, 0.1088, 0.16, 0.1792, 0.2136, 0.2352, 0.30, 0.32, 0.34, 0.35]
bracket = [0.0, 8952.49, 75984.55, 133536.07, 155229.80, 185852.57, 374837.88, 590795.99, 1127926.84, 1503902.46, 4511707.37, 9e99]
df_expanded = summary_stats(df_expanded, 'tot_inc', rate, bracket, "HHS Unadjusted")
df_hhs_new = summary_stats(df_hhs_new, 'tot_inc', rate, bracket, "HHS Adjusted")

end = time.time()
print ("Time elapsed numba:", end - start)
survival_plot(df_expanded,'tot_inc')
survival_plot(df_hhs_new,'tot_inc')


df_hhs_new.to_csv("pit_mexico_big_adjusted.csv")
