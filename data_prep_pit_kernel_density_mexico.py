...
"""
# Load dataset
df = pd.read_csv('pit_mexico_big.csv')

plot_density_chart(df, 'tot_inc', logx=True)

# Remove missing or zero values in Employment_Income
df = df[df["tot_inc"].notna()]
df = df[df["tot_inc"] > 0]
...
pit_syn = pd.DataFrame({"Year": 2022, "tot_inc": synthetic_samples, 'weight':1.0})

plot_density_chart(pit_syn, 'tot_inc', logx=True)

pit_syn = pit_syn.sort_values(by=['tot_inc'])

pit_syn = pit_syn.reset_index(drop=True)

pit_syn['id_n'] = pit_syn.index

pit_syn = pit_syn[['id_n', 'Year', 'tot_inc', 'weight']]

pit_syn.to_csv('pit_mexico_syn.csv')
